<template>
    <div class="AlbumBox" v-if="props.ifShow">
        <div class="Background">
            <div class="Title">
                <p class="text-lg font-medium">选择合集</p>
            </div>
        ...
        <div class="Content">
            <p class="text-left text-lg font-medium mt-5">添加合集</p>
            ...
            <div v-for="(item, index) in albumList" :key="index"
                ...
                @click="changeselectedAlbum(index)" :class="{ 'bg-gray-100': selectedAlbum.includes(item) }">
                ...
                <div class="flex justify-center items-start flex-col w-20">
                    ...
                    <!-- <p class="text-left text-lg font-medium">{{ item.price }}</p> -->
                </div>
            </div>
...
const emit = defineEmits(['updateIfShow','updateSelectedBlindBox']);
...
//     return selectedAlbum.value.includes(item);
// };
...
// 点击改变选中后的合集selectedAlbum
const changeselectedAlbum = (index: number) => {
    const selectedItem = albumList.value[index];
    if (selectedAlbum.value.includes(selectedItem)) {
        selectedAlbum.value = selectedAlbum.value.filter(item => item !== selectedItem);
    } else {
        selectedAlbum.value.push(selectedItem);
    }
};
// 点击改变选中后的合集synthesisedCollection
...
const toggleVisibility = () => {
    ...
    emit('updateSelectedBlindBox', selectedAlbum.value);
};
